          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {messages.length === 0 && !insightMutation.isPending && (
              <div className="h-full flex flex-col items-center justify-center text-center py-10">
                ...
                <p className="text-slate-500 text-sm mb-6">Start a conversation about your fitness data</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 w-full max-w-md">
                  {SUGGESTIONS.map((s) => (
                    <button
                      key={s}
                      onClick={() => send(s)}
                      className="text-left text-xs text-slate-300 bg-white/5 hover:bg-white/10 border border-line rounded-lg px-3 py-2.5 transition-colors cursor-pointer"
                    >
                      {s}
                    </button>
                  ))}
            ...
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm leading-relaxed whitespace-pre-wrap ${
                  msg.role === "user"
                    ? "bg-emerald-400 text-[#04130C] rounded-br-sm"
                    : "bg-white/5 text-slate-200 border border-line rounded-bl-sm"
                }`}>
                  {msg.content}
                </div>
              </div>
